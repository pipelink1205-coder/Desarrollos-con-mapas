-- Una sola pareja de coordenadas: latitud / longitud (las que usa Admin y el mapa).
-- Fusiona datos legacy, elimina latitud_verdadera / longitud_verdadera del Excel.
-- Ejecutar en Supabase después de 05 (y 09 si ya lo corriste; este script lo reemplaza).

-- 1) Conservar el mejor dato en latitud/longitud
UPDATE public.instituciones
SET
  latitud  = COALESCE(latitud, latitud_verdadera),
  longitud = COALESCE(longitud, longitud_verdadera)
WHERE latitud_verdadera IS NOT NULL OR longitud_verdadera IS NOT NULL;

-- 2) Vistas sin columnas duplicadas
...
ALTER TABLE public.instituciones
  DROP COLUMN IF EXISTS latitud_verdadera,
  DROP COLUMN IF EXISTS longitud_verdadera;
